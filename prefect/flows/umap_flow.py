...
def umap_analysis_flow(
    ...
    print("UMAP Analysis starting in Prefect Cloud...")
    ...
    combined_df, debug_messages = load_and_prepare_data(
        stored_files=stored_files, selected_ids=selected_ids, sample_frac=sample_frac
    )
    ...
    cluster_labels, clustering_info = compute_clustering(
        umap_embedding=umap_embedding, eps=dbscan_eps, min_samples=dbscan_min_samples
    )
