...
def run_flow_async(flow_id, **kwargs):
    ...
    try:
        ...
        # Import the flow here to ensure it uses the correct API URL
        from sculpt.flows.umap_flow import umap_analysis_flow
        
        # Create a uniquely named wrapper flow for Prefect tracking
        @flow(name=f"UMAP-{flow_id}", log_prints=True)
        def tracked_umap_flow(**params):
            return umap_analysis_flow(**params)
        ...
        print(f"Check http://localhost:4200 for details")
        print("="*60)
...
def run_umap_analysis(
    ...
    if triggered == "run-umap":
        ...
        if not stored_files or not selected_ids:
            return (
                {},
                html.Div("Please upload and select files first", style={"color": "orange"}),
                no_update,
                no_update,
                no_update
            )
        ...
        print(f"Processing {len(processed_files)} selected files from {len(stored_files)} total files")
        ...
        print(f"Selected features: {features[:5]}..." if len(features) > 5 else f"Selected features: {features}")
        ...
        return (
            empty_fig,
            html.Div(f"UMAP analysis started ({env_label})...", style={"color": "blue"}),
            no_update,
            html.Div("Processing..."),
    ...
    # Check progress on interval
        ...
        if result["status"] == "completed":
            ...
            if not flow_result or not flow_result.get("success"):
                error_msg = flow_result.get("error", "Unknown error") if flow_result else "No results"
                return (
                    {},
            ...
            metrics_html = html.Div([
                ...
                html.P(f"Computation Time: {metadata.get('computation_time', 0):.2f}s")
            ])
