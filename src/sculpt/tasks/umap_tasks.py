        ...
        if file_dict["id"] in selected_ids_int:
            try:
                ...
                if sample_frac < 1.0:
                    ...
                    logger.info(f"Sampled {len(df)} points from {file_dict['filename']}")
                ...
                dfs.append(df)
                debug_messages.append(f"Loaded {file_dict['filename']}: {len(df)} events")
                
            except Exception as e:
                logger.warning(f"Error loading file {file_dict.get('filename', 'unknown')}: {str(e)}")
                ...
                try:
                    ...
                    if isinstance(file_dict["data"], str):
                        ...
                        df = pd.DataFrame(data_dict["data"] if "data" in data_dict else data_dict)
                        ...
                        df["file_id"] = file_dict["id"]
                        dfs.append(df)
                        logger.info(f"Successfully loaded {file_dict['filename']} with alternative parsing")
                except Exception as e2:
                    logger.error(f"Failed to load {file_dict.get('filename', 'unknown')}: {str(e2)}")
                    debug_messages.append(f"Failed to load {file_dict.get('filename', 'unknown')}")
...
@task(
    ...
    cache_key_fn=lambda context, parameters, **kwargs: 
        f"umap_{len(parameters['feature_data'])}_{parameters['num_neighbors']}_{parameters['min_dist']}_{hash(str(parameters['feature_cols']))}",
...
def compute_umap_embedding(
    ...
    logger.info(f"Starting UMAP: {len(feature_data)} samples, {len(feature_cols)} features")
...
@task(
    name="create_umap_dataframe",
    description="Create final UMAP dataframe"
)
