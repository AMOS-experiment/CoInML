...
def convert_feature_for_display(data, feature_name):
    ...
    if (feature_name.startswith('energy_') or 
        feature_name in ['KER', 'EESum', 'TotalEnergy']):
        converted_data = np.array(data) * AU_TO_EV
        unit_label = 'eV'
        
    # Angle features - convert to degrees  
    elif (feature_name.startswith('theta_') or 
          feature_name.startswith('phi_') or
          'relative_angle' in feature_name or
          'angle_' in feature_name):
